...
use super::widgets::{
    format_cache_ratio, format_cost, format_tokens, get_client_display_name,
    get_provider_display_name, get_provider_shade, scrollbar_state,
...
pub fn render(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    render_timeline_list(frame, app, area);
}
...
fn timeline_row_label(app: &App, row: &TimelineRowData, layout: TimelineTableLayout) -> String {
    if app.timeline_granularity == TimelineGranularity::Day && layout.time_width < 12 {
        if let Some(period) = &row.period {
            return period.start.format("%m-%d").to_string();
        }
    }
    row.label.clone()
}
...
mod tests {
    ...
    #[test]
    fn narrow_timeline_compacts_day_when_table_would_not_fit() {
        let mut app = make_app(58);
        let body = render_body(&mut app, 58, 12);
        assert!(
            !body.contains("2026-05-29"),
