...
//! `Settings::load()` needs so it can read
//! `~/Library/Application Support/tokscale/` once on upgrade.

use std::path::PathBuf;

pub use tokscale_core::paths::{
    get_cache_dir, get_config_dir, is_config_dir_overridden, legacy_dot_cache_tokscale_dir,
};
...
/// Used by `Settings::load()` so users upgrading
