...
pub fn run_models_report(args: ModelsReportArgs) -> Result<()> {
    ...
    if json {
        ...
        if commands::light_cache::resolve_should_write_cache(write_cache, no_write_cache, &settings)
        {
            commands::light_cache::write(&home_dir, &clients, &since, &until, &year, &group_by);
        }
    }
