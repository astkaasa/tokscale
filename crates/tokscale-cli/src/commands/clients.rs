...
pub(crate) fn run(json: bool, home_dir: Option<String>) -> Result<()> {
    let explicit_home_dir = home_dir;
    let use_env_roots = explicit_home_dir.is_none();
    let scanner_settings = crate::tui::settings::load_scanner_settings_for_home(&explicit_home_dir);
    ...
    struct ClientRow {
        ...
        #[serde(skip_serializing_if = "Vec::is_empty")]
        diagnostics: Vec<crate::claude_diagnostics::ClientDiagnostic>,
    }
        ...
        ClientId::iter()
            .map(|client| {
                ...
                let label = match client {
                    ...
                    _ => crate::tui::client_ui::display_name(client),
                }
                .to_string();
                ...
                let diagnostics = if client == ClientId::Claude {
                    crate::claude_diagnostics::diagnostics_for_clients_row(&home_dir)
                } else {
                    Vec::new()
                };
