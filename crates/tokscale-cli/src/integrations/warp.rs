...
pub fn run_cli_command(subcommand: crate::cli::WarpSubcommand) -> Result<()> {
    match subcommand {
        crate::cli::WarpSubcommand::Login { token, cookie } => run_warp_login(token, cookie),
        crate::cli::WarpSubcommand::Logout { purge_cache } => run_warp_logout(purge_cache),
        crate::cli::WarpSubcommand::Status { json } => run_warp_status(json),
        crate::cli::WarpSubcommand::Sync { json } => run_warp_sync(json),
    }
}
