...
pub fn run_cli_command(subcommand: crate::cli::TraeSubcommand) -> anyhow::Result<()> {
    use colored::Colorize;
    let rt = tokio::runtime::Runtime::new()?;

    match subcommand {
        crate::cli::TraeSubcommand::Login { manual, variant } => {
            if manual {
                use std::io::{self, Write};
                let selected =
                    parse_variant_arg(variant.as_deref())?.unwrap_or(auth::TraeVariant::Solo);
                println!();
                ...
                auth::save_manual_token(selected, token, None)?;
                println!(
                    "\n  {}",
                    format!("Token saved for {}", selected.client_str()).green()
                );
            } else {
                let variants: Vec<auth::TraeVariant> = match parse_variant_arg(variant.as_deref())?
                {
                    Some(v) => vec![v],
                    None => auth::all_variants().to_vec(),
                };

                let mut any_success = false;
                for v in variants {
                    match rt.block_on(auth::resolve_token(v)) {
                        Ok(_) => {
        ...
        crate::cli::TraeSubcommand::Logout { variant } => {
            let variants: Vec<auth::TraeVariant> = match parse_variant_arg(variant.as_deref())? {
                Some(v) => vec![v],
                None => auth::all_variants().to_vec(),
            };
        ...
        crate::cli::TraeSubcommand::Status { json } => {
            let mut status = serde_json::Map::new();
            for v in auth::all_variants() {
                let has = auth::has_credentials(v);
                if json {
        ...
        crate::cli::TraeSubcommand::Sync { since, include_aux } => {
            let days = since.unwrap_or(30);
            if days <= 0 {
                anyhow::bail!("--since must be a positive number of days (got {days})");
            }
            let variants: Vec<auth::TraeVariant> = auth::all_variants()
                .into_iter()
                .filter(|v| auth::has_credentials(*v))
                .collect();
            rt.block_on(sync::run_trae_sync(&variants, days, include_aux))
        }
    }
}

fn parse_variant_arg(arg: Option<&str>) -> anyhow::Result<Option<auth::TraeVariant>> {
    match arg {
        Some("solo") => Ok(Some(auth::TraeVariant::Solo)),
        Some("ide") => Ok(Some(auth::TraeVariant::Ide)),
...
mod tests {
    ...
    #[test]
    fn parse_variant_arg_accepts_known_values() {
        assert_eq!(
            parse_variant_arg(Some("solo")).unwrap(),
            Some(auth::TraeVariant::Solo)
        );
        assert_eq!(
            parse_variant_arg(Some("ide")).unwrap(),
            Some(auth::TraeVariant::Ide)
        );
    }

    #[test]
    fn parse_variant_arg_none_when_omitted() {
        assert_eq!(parse_variant_arg(None).unwrap(), None);
    }

    #[test]
    fn parse_variant_arg_rejects_unknown_value() {
    ...
    #[test]
    fn parse_variant_arg_rejects_empty_string() {
